...
LineNumberTreeProcessor, link:lib/line-number-treeprocessor.rb[]::
Adds line numbering and file references to allow connecting between source document and output. Should be called with the sourcemap preprocessor included as a separate extension and also with reference to the templates folder included. See link:lib/line-number-treeprocessor/sample.adoc[`sample.adoc`] for further information.
